...
sub store_factor ($rem, $f, $factors) {
    ...
    if (is_prime($f)) {
        ...
        my $f_rem = find_small_factors($f, $factors);
...
sub find_small_factors ($rem, $factors) {
    ...
    my @factorization_methods = (
        ...
        sub {
            if ($len < 3000) {
                ...
                lucas_miller_factor($rem, +1, ($len > 1000) ? 15 : LUCAS_MILLER_ITERATIONS);
            }
        },
  ...
  MAIN_LOOP: for (; ;) {
        ...
        if (   (Math::GMPz::Rmpz_popcount($rem + 1) == 1)
            or (Math::GMPz::Rmpz_popcount($rem - 1) == 1)
            or (Math::GMPz::Rmpz_popcount(($rem * 3) - 1) == 1)
            or (Math::GMPz::Rmpz_popcount(($rem * 3) + 1) == 1)) {
            return $rem;
        }
...
sub find_prime_factors ($n, $factors) {
    ...
    if (length("$n") > 65) {
        find_all_prime_factors($n, $factors);
        return 1;
    }
...
sub find_all_prime_factors ($n, $factors) {
    ...
    state $t = do {
        ...
        Math::GMPz::Rmpz_primorial_ui($z, 1e7);
        $z;
    };
    ...
    if ($g > $rem or ($g > ~0 and length("$rem") < 80)) {
        ...
        push @$factors, map { Math::GMPz->new("$_") } ntheory::factor($g);
        push @$factors, map { Math::GMPz->new("$_") } ntheory::factor($rem);
        return 1;
    }

    if (length("$n") > 50) {

        say ":: Factoring with FactorDB...";

        my @f;
        if (SCRAPE_FACTORDB) {
            chomp(@f = `$^X /home/swampyx/Other/Programare/experimental-projects/factordb/scrape_factordb.pl $n`);
        }
        else {
            chomp(@f = `$^X /home/swampyx/Other/Programare/experimental-projects/factordb/get_factordb.pl $n`);
        }

        if (Math::GMPz->new(vecprod(@f)) == $n and vecall { is_prime($_) } @f) {
            push @$factors, @f;
            return 1;
        }
    ...
    if (length("$n") < 70) {
        say ":: Factoring with ntheory (<70 len)...";
        push @$factors, map { Math::GMPz->new("$_") } Math::Prime::Util::GMP::factor($n);
        return 1;
    }

    say ":: Too large to factor...";
    return 0;
}
...
sub factorize ($n) {
    ...
    my @divisors = ((LOOK_FOR_SPECIAL_FORMS and $len > 65) ? special_form_factorization($n) : ());
...
sub prefactorization ($n, $factors) {
    ...
    if (length("$n") <= 45) {
        ...
        push @$factors, Math::Prime::Util::GMP::factor($n);
        return 1;
    }
