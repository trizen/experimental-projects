...
use ntheory qw(kronecker forcomb forprimes kronecker divisors factor_exp divisor_sum euler_phi lucas_sequence random_prime is_square_free);
...
sub fibonacci_pseudoprimes ($limit, $callback) {
    ...
    forprimes {
        ...
        foreach my $d (divisors($p - kronecker($p, 5))) {
            ...
            my ($U, $V) = lucas_sequence($p, 1, find_D($p), $d);
