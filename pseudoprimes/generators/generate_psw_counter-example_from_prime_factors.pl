...
sub lucas_znorder ($P, $Q, $p) {
    my $D = $P*$P - 4*$Q;
    foreach my $d (divisors($p - kronecker($D, $p))) {
        if (lucasumod($P, $Q, $d, $p) == 0) {
            return $d;
        }
    }
    return undef;
    ...
    sub ($m, $lambda1, $lambda2, $j, $k) {
        ...
        if ($k == 1) {
            ...
            foreach my $i ($j .. $end) {
                ...
                if (($t - 1) % $lambda1 == 0 and ($t - 1) % znorder(2, $p) == 0) {
                    say $t;
                    if (($t + 1) % $lambda2 == 0 and ($t + 1) % lucas_znorder(1, -1, $p) == 0) {
                        say ":: Found PSW counter-example: $t";
                        $callback->($t);
                    }
                }
            }
        ...
        foreach my $i ($j .. $end) {
            ...
            my $L1 = lcm($lambda1, znorder(2, $p));
            gcd($L1, $m) == 1 or next;

            my $L2 = lcm($lambda2, lucas_znorder(1, -1, $p));
            gcd($L2, $m) == 1 or next;
...
while (<>) {
    ...
    is_smooth($p+1, 8*logint($p, 2)) || next;
    is_smooth($p-1, 8*logint($p, 2)) || next;
    #$p > 2**64 or next;
    push @primes_620, $p;
}
...
foreach my $k ((5..scalar(@primes_620))) {
    ...
    foreach my $group (@groups) {
        ...
        carmichael_numbers_in_range(Math::GMPz->new(10)**20, $k, $group, sub ($n) { say $n; say $fh $n; });
    }
}
