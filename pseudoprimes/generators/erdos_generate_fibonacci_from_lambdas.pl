...
# This version generates Fibonacci pseudoprimes and other Lucas pseudoprimes.
...
sub lambda_primes ($L) {
    ...
    #my $D = 19;              # 5, -7, 9, -11, 13, -15, 17, -19, 21
    my $D = 5;              # 5, 12, 21, 32, 45, 60, 77, 96

    uniq(

    #~ (grep { ($_ > 2) && ($L % $_ != 0) && is_prime($_) && ($L % ($_ - kronecker(5, $_)) == 0) } map { $_ - 1 } @D),
    #~ (grep { ($_ > 2) && ($L % $_ != 0) && is_prime($_) && ($L % ($_ + kronecker(5, $_)) == 0) } map { $_ + 1 } @D)

    #~ (grep { ($_ > 2) and ($L % $_ != 0) and kronecker($D, $_) == -1 and ($L % ($_ - kronecker($D, $_)) == 0) and is_smooth($_-1, 1e2) } map { $_ + 1 } grep { is_prime($_+1) } @D),
    #~ (grep { ($_ > 2) and ($L % $_ != 0) and kronecker($D, $_) == -1 and ($L % ($_ - kronecker($D, $_)) == 0) and is_smooth($_-1, 1e2) } map { $_ - 1 } grep { is_prime($_-1) } @D)

    #~ (grep { ($_ > 2) and ($L % $_ != 0) and kronecker($D, $_) == -1 and ($L % ($_ - kronecker($D, $_)) == 0) and is_smooth($_-1, (factor($_+1))[-1]) } map { $_ + 1 } grep { is_prime($_+1) } @D),
    #~ (grep { ($_ > 2) and ($L % $_ != 0) and kronecker($D, $_) == -1 and ($L % ($_ - kronecker($D, $_)) == 0) and is_smooth($_-1, (factor($_+1))[-1]) } map { $_ - 1 } grep { is_prime($_-1) } @D)

    (grep { ($_ > 2) and ($L % $_ != 0) and (kronecker($D, $_) == -1) and ($L % ($_ - kronecker($D, $_)) == 0) } map { $_ + 1 } grep { is_prime($_+1) } @D),
    (grep { ($_ > 2) and ($L % $_ != 0) and (kronecker($D, $_) == -1) and ($L % ($_ - kronecker($D, $_)) == 0) } map { $_ - 1 } grep { is_prime($_-1) } @D)
