...
sub generate($n) {
    ...
    my $min_m = 48690327298470;

    for (my $j = 0 ; ; ++$j) {

        if ($m > $min_m and vecall { is_prime(Math::Prime::Util::GMP::addint(Math::Prime::Util::GMP::vecprod($m, $PERFECT_N, $_), 1)) } @DIVISORS) {
            return $m;
        }
...
__END__
Searching for a(5) with m = 48690327298470
Performance: 422.965373836406 * 10^9 terms per second
