...
// Compilation:
//  g++ -march=native -Ofast -lgmp search_a12.cpp -o x
...
int main() {
    ...
    cout << "Test: " << (from * mul * (1 << (n - 4)) * 9 * (1 << (n - 5)) + 1) << endl;
    ...
    for (u64 k = from ; ; k++) {
        ...
        if (k % 1000000000LL == 0LL) {
            cout << "Tested up to k = " << k << " -- where largest p = " << (((multiplier * k * 8LL) << (n - 6)) + 1) << endl;
        }

        u64 m = k * multiplier;

        if (primality_pretest(6 * m + 1) && primality_pretest(12 * m + 1) && primality_pretest(18 * m + 1) && is_chernick(n-4, m, z)) {
