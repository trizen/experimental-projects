...
// Compilation:
//  g++ -march=native -Ofast -lgmp find_a12.cpp -o x
...
int main() {
    ...
    // sanity checks (all three must be 1)
    ...
    u64 from = 6269000000000;       // initial value of k

    int n = 12;
    int t = n - 4;

    // Make sure we don't overflow
    cout << "Test: " << (from * 5 * (1 << (n - 4)) * 9 * (1 << (n - 5)) + 1) << endl;
    cout << "Test: " << (((((from * 5) << t) * 9) << (n - 5)) + 1) << endl;

    // Make sure mpz works
    mpz_set_ui(z, (((((from * 5) << t) * 9) << (n - 5)) + 1));
    ...
    for (u64 k = from ; ; k++) {
        ...
        if (k % 1000000000 == 0) {
            cout << "Tested up to k = " << k << " -- where largest p = " << (((((k * 5) << t) * 9) << (n - 5)) + 1) << endl;
        }

        u64 m = k * multiplier;

        if (primality_pretest(6 * m + 1) && primality_pretest(12 * m + 1) && primality_pretest(18 * m + 1) && is_chernick(n-3, m, z)) {
            cout << ":: Candidate for k = " << k << " and m = " << m << endl;
            //break;
        }
    }
