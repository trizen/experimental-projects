...
forprimes {
    if (($_ % 80 == 3) ) {
        foreach my $d (divisors($_ - 1)) {
            #next if ($d+1 == $_);
            next if ($d <= 7);
            if (powmod(2, $d, $_) == 1 or powmod(3, $d, $_) == 1 or powmod(5, $d, $_) == 1 or powmod(7, $d, $_) == 1 or powmod(11, $d, $_) == 1) {
...
foreach my $arr (values %common_divisors) {
    ...
    forcomb {
       ...
       # if ($n > ~0) {
       if (is_pseudoprime($n, 2) or is_pseudoprime($n, 3) or is_pseudoprime($n, 5) or is_pseudoprime($n, 7) or is_pseudoprime($n, 11)) {
            say $n;
        }
       # }
    } scalar(@nums), $k;
