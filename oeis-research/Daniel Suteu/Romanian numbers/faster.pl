...
# a(n) is the smallest positive number k such that the Romanian text of k contains n digits.
...
foreach my $k(1..1e7) {
    ...
    my $n = length(number_to_ro($k) =~ tr/, -//dr);

    if (not exists $table{$n}) {
        $table{$n} = $k;
    }
}
...
a(66) = 2244444 -> două milioane două sute patruzeci și patru de mii patru sute patruzeci și patru
a(67) = 2444444 -> două milioane patru sute patruzeci și patru de mii patru sute patruzeci și patru
a(68) = 4444444 -> patru milioane patru sute patruzeci și patru de mii patru sute patruzeci și patru
